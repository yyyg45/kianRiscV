...
`endif
    ...
    input wire [$clog2(NUM_SETS)-1:0] idx,
    input wire [ADDR_WIDTH-$clog2(NUM_SETS)-$clog2(LINE_BYTES)-1:0] tag,
  ...
  initial begin
    if (WAYS != 4)
      $fatal(1, "cache_sram_I$: this implementation supports WAYS=4 only (got %0d).", WAYS);

    if (!is_pow2(NUM_SETS))
      $fatal(1, "cache_sram_I$: NUM_SETS (%0d) must be a power-of-two.", NUM_SETS);

    if (NUM_SETS < 2) $fatal(1, "cache_sram_I$: NUM_SETS (%0d) must be >= 2.", NUM_SETS);

    if (!is_pow2(LINE_BYTES))
      $fatal(1, "cache_sram_I$: LINE_BYTES (%0d) must be a power-of-two.", LINE_BYTES);
  ...
  (* keep *) reg [NUM_SETS-1:0] valid0, valid1, valid2, valid3;

  (* keep *) reg [3*NUM_SETS-1:0] plru;
  ...
  wire hit_comb = way0_match | way1_match | way2_match | way3_match;
  ...
  always @(posedge clk) begin
    if (!resetn) begin
      valid0 <= {NUM_SETS{1'b0}};
      valid1 <= {NUM_SETS{1'b0}};
      valid2 <= {NUM_SETS{1'b0}};
      valid3 <= {NUM_SETS{1'b0}};
      plru   <= {(3 * NUM_SETS) {1'b0}};
