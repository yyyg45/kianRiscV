...
static uint8_t _sd_send_command(uint8_t cmd, uint32_t arg)
{
	...
	// If non-SDHC card, use byte addressing rather than block (512)
	// addressing
	if (_sdhc_card == 0) {
		switch (cmd) {
...
static int _sd_init(void)
{
	...
	do {
		...
		response =
		        _sd_send_command(CMD8_SEND_IF_COND, CMD8_3V3_MODE_ARG);
		if (retries++ > MAX_RETRIES) {
	...
	do {
		...
		response = _sd_send_command(ACMD41_SD_SEND_OP_COND,
		                            ACMD41_HOST_SUPPORTS_SDHC);
...
int sd_readsector(uint32_t start_block, uint8_t *buffer, uint32_t sector_count)
{
	...
	while (sector_count--) {
		// Request block read
		response = _sd_send_command(CMD17_READ_SINGLE_BLOCK,
		                            start_block++);
		if (response != 0x00) {
		...
		while (spi_sendrecv(0xFF) != CMD_START_OF_BLOCK) {
			...
			if (timer_diff(timer_now(), tStart) >= 1000) {
				printf("sd_readsector: Timeout waiting for "
				       "data ready\n");
				return 0;
			}
		}
...
int sd_writesector(uint32_t start_block, uint8_t *buffer, uint32_t sector_count)
{
	...
	while (sector_count--) {
		// Request block write
		response = _sd_send_command(CMD24_WRITE_SINGLE_BLOCK,
		                            start_block++);
		if (response != 0x00) {
		...
		while (spi_sendrecv(0xFF) == 0) {
			// Timeout
			if (timer_diff(timer_now(), tStart) >= 1000) {
				printf("sd_writesector: Timeout waiting for "
				       "data write complete\n");
				return 0;
			}
		}
