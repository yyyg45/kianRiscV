...
set_property -dict {PACKAGE_PIN M6 IOSTANDARD LVCMOS33} [get_ports ext_resetn]
#set_property PULLUP TRUE [get_ports ext_resetn]

# Reset timing constraints
set_false_path -from [get_ports ext_resetn]

# Synchronizer registers
#set_property ASYNC_REG TRUE [get_cells -hier -filter {NAME =~ *clk_locked_sync*}]
#set_property ASYNC_REG TRUE [get_cells -hier -filter {NAME =~ *rst_n_sync*}]
...
set_property PULLTYPE PULLUP [get_ports {flash_csn}]
...
# Bitstream configuration for proper power-on reset
set_property BITSTREAM.CONFIG.CONFIGRATE 33 [current_design]
set_property BITSTREAM.CONFIG.SPI_BUSWIDTH 4 [current_design]
set_property BITSTREAM.GENERAL.COMPRESS TRUE [current_design]

# Critical for power-on behavior
set_property BITSTREAM.CONFIG.SPI_FALL_EDGE YES [current_design]
set_property CONFIG_VOLTAGE 3.3 [current_design]
set_property CFGBVS VCCO [current_design]

# Force all flip-flops to initialize to their INIT values at power-on
set_property BITSTREAM.STARTUP.STARTUPCLK CCLK [current_design]

# Add delay after configuration before releasing DONE
set_property BITSTREAM.STARTUP.LCK_CYCLE 6 [current_design]
set_property BITSTREAM.STARTUP.MATCH_CYCLE NoWait [current_design]

# Keep DONE pin low longer after configuration
