...
unsigned int heap_memory_used = 0;

static void printf_c(char c)
{
	uart_print_char(c);
}
...
void printf_x(unsigned int n, int width)
{
	...
	// Print leading zeros if width is specified and larger than the number
	// length
	for (; i < width; i++) {
		uart_print_char('0');
	}
...
char *strcpy(char *dst, const char *src)
{
	...
	while (1) {
		...
		if (__builtin_expect((((v)-0x01010101UL) & ~(v) & 0x80808080UL),
		                     0)) {
			dst[0] = v & 0xff;
			if ((v & 0xff) == 0) {
				return r;
...
int strcmp(const char *s1, const char *s2)
{
	...
	while (1) {
		...
		if (__builtin_expect(
		            (((v1)-0x01010101UL) & ~(v1) & 0x80808080UL), 0)) {
			return 0;
		}
